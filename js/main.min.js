...
btn.onclick = function () {
  btn.classList.toggle('button-open');
  var navres = document.getElementById("navbarresponsive");

  if (navres.classList.contains('hidenav') && navres.classList.contains('none')) {
    navres.classList.remove('none');
    navres.classList.replace('hidenav', 'shownav');
  } else if (navres.classList.contains('shownav')) {
    navres.classList.replace('shownav', 'hidenav');
    navres.classList.add('none');
  }
};
"use strict";
...
var gblock = document.querySelectorAll('.gallery-block');
/*heart icon*/

for (var i = 0; i < gblock.length; i++) {
  var heart = gblock[i].querySelector('#heart');
  ...
  var heartNum = gblock[i].querySelector('#heartnum');
  var heartVal = heartNum.getAttribute('value');
  heartNum.insertAdjacentHTML('beforeend', heartVal);
  var prodName = gblock[i].querySelector('.good-name');
  var brandName = gblock[i].querySelector('.good-brand');
  var prodNameAttr = prodName.getAttribute('data-prodname');
  var brandNameAttr = brandName.getAttribute('data-name');
...
function changeval() {
  ...
  var rrng = document.querySelector('#rrng').value;
  var lrng = document.querySelector('#lrng').value;
  from.value = lrng;
  to.value = rrng;
}
...
function changenum() {
  ...
  var rrng = document.querySelector('#rrng');
  var lrng = document.querySelector('#lrng');
  rrng.value = to;
  lrng.value = from;
} //changenum
...
for (var _i = 0; _i < gblock.length; _i++) {
  var plast = gblock[_i].querySelector('.price-last');

  var pnow = gblock[_i].querySelector('.price-now');

  var plastAt = gblock[_i].getAttribute('data-oldprice');

  var pnowAt = gblock[_i].getAttribute('data-price');

  var plastcur = plast.querySelector('#cur');
  var pnowcur = pnow.querySelector('#cur');
  var rub = '<i class="fa fa-ruble-sign"></i>';
  ...
  var inequality = plastAt - pnowAt;
  var inqdivide = inequality / plastAt;
  var resultMulti = inqdivide * 100;
  var result = parseInt(resultMulti);

  var disCount = gblock[_i].querySelector('#disCount');

  gblock[_i].setAttribute('data-discount', result);
...
for (var _i2 = 0; _i2 < gblock.length; _i2++) {
  var rating = gblock[_i2].querySelector('.rating');

  var ratingAt = gblock[_i2].getAttribute('data-rating');

  var fis = '<i class="bi bi-star-fill"></i>';
  var is = '<i class="bi bi-star"></i>';
...
window.onload = function () {
  document.querySelector('button#sortAsc').onclick = function () {
    sortList("data-price");
  };

  document.querySelector('button#sortDesc').onclick = function () {
    sortListDesc("data-price");
  };

  document.querySelector('button#sortRating').onclick = function () {
    sortListDesc("data-rating");
  };

  document.querySelector('button#sortDiscount').onclick = function () {
    sortListDesc("data-discount");
  };

  function sortList(sortType) {
    var sortwrapper = document.querySelector('.gallery-wrap');

    for (var _i3 = 0; _i3 < sortwrapper.children.length - 1; _i3++) {
      for (var j = _i3; j < sortwrapper.children.length; j++) {
        if (+sortwrapper.children[_i3].getAttribute(sortType) > +sortwrapper.children[j].getAttribute(sortType)) {
          console.log(1);
          var replacedNode = sortwrapper.replaceChild(sortwrapper.children[j], sortwrapper.children[_i3]);
          insertAfter(replacedNode, sortwrapper.children[_i3]);
        }
      }
    }
  }
  ...
  function sortListDesc(sortType) {
    ...
    for (var _i4 = 0; _i4 < sortwrapper.children.length - 1; _i4++) {
      for (var j = _i4; j < sortwrapper.children.length; j++) {
        if (+sortwrapper.children[_i4].getAttribute(sortType) < +sortwrapper.children[j].getAttribute(sortType)) {
          console.log(1);
          var replacedNode = sortwrapper.replaceChild(sortwrapper.children[j], sortwrapper.children[_i4]);
          insertAfter(replacedNode, sortwrapper.children[_i4]);
        }
      }
    }
  }
...
var inp = document.querySelector('.search-container');
...
$(function () {
  $('.mail').html('<svg width="2em" height="2em" viewBox="0 0 16 16" class="bi bi-envelope" fill="currentColor" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M0 4a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2V4zm2-1a1 1 0 0 0-1 1v.217l7 4.2 7-4.2V4a1 1 0 0 0-1-1H2zm13 2.383l-4.758 2.855L15 11.114v-5.73zm-.034 6.878L9.271 8.82 8 9.583 6.728 8.82l-5.694 3.44A1 1 0 0 0 2 13h12a1 1 0 0 0 .966-.739zM1 11.114l4.758-2.876L1 5.383v5.73z"/></svg>');
  $('.plus').html('<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#FFF" class="bi bi-plus-circle" viewBox="0 0 16 16"><path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14zm0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16z"/><path d="M8 4a.5.5 0 0 1 .5.5v3h3a.5.5 0 0 1 0 1h-3v3a.5.5 0 0 1-1 0v-3h-3a.5.5 0 0 1 0-1h3v-3A.5.5 0 0 1 8 4z"/></svg>');
...
var locModal = document.querySelector('#locModal');
...
function change() {
  var sel = document.querySelector('#location').selectedIndex;
  var opt = document.querySelector('#location').options;
...
var mailWindow = document.querySelector('.mail-container');
...
var dropdownmobile = document.getElementById('dropdownMobile');
var menumob = document.getElementById('dropdownMenuMobile');

dropdownmobile.onmouseover = function (event) {
  var target = event.target;
...
menumob.onmouseleave = function (event) {
  var target = event.target;
...
var multiItemSlider = function () {
  return function (selector, config) {
    ...
    _sliderWrapper = _mainElement.querySelector('.custom-slider__wrapper'),
        // обертка для .slider-item
    _sliderItems = _mainElement.querySelectorAll('.custom-slider__item'),
        // элементы (.slider-item)
    _sliderControls = _mainElement.querySelectorAll('.custom-slider__control'),
        // элементы управления
    _sliderControlLeft = _mainElement.querySelector('.custom-slider__control_left'),
        // кнопка "LEFT"
    _sliderControlRight = _mainElement.querySelector('.custom-slider__control_right'),
        // кнопка "RIGHT"
    ...
    var _controlClick = function _controlClick(e) {
      if (e.target.classList.contains('custom-slider__control')) {
        e.preventDefault();
        var direction = e.target.classList.contains('custom-slider__control_right') ? 'right' : 'left';
